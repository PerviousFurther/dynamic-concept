...
struct programable 
{
    ...
    // btw, you can also add `const` to qualify or `noexcept` to strength `concept`.

    void program() { dynamic::invoke<0>(this); }
    void program_unsafe() { dynamic::invoke<1>(this); }
    void program_unsafe(int v) { dynamic::invoke<eprogram>(this, v); }
};
...
dyn::generic<programmer, std::shared_ptr<void>> somewhat(int index)
{
    switch (index)
    {
